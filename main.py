...
async def on_voice_state_update(member, before, after):
    ...
    if before.channel is None and after.channel is not None:
        ...
        "voice channel joined": f'`{after.channel}`',
        "timestamp": f'`{current_date_utc}`'
        }

        await bot_log_channel_name.send(voice_log_dict)
        logger.info(f'{full_username_with_discriminator} has joined {after.channel} at {current_date_utc}')
    else:
